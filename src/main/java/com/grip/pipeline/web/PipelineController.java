 ...
 *
 * <p>
 * The user is derived from the {@code sub} claim of the Supabase JWT, so no
 * endpoint accepts a user id from the caller — a token holder can only read
 * their own pipeline.
 */
@RestController
@RequestMapping("/api/pipeline")
@Tag(name = "Pipeline analytics", description = "Read-only hiring-pipeline analytics over Grip's contacts and status_events. "
